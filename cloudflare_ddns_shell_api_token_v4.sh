...
while [ $retry_count -lt $max_retries ]; do
  if [ "$record_ip" = "$record_ip_check" ] && [ "$update_record" = "true" ]; then
    ...
    # 记录错误时间和对应次数
    echo "$(date +"%Y-%m-%d %H:%M:%S") 错误第 $retry_count 次" > /var/log/cloudflare_ddns_shell_api_token_v4_error.log
    echo "$update_record_response" > /var/log/cloudflare_ddns_shell_api_token_v4_error.log
    echo "$record_ip_check_response" > /var/log/cloudflare_ddns_shell_api_token_v4_error.log
    exec "$0" "$@"
  fi
done
...
From: $sender_email
To: $recipient_email
Subject: [server] cloudflare_ddns_shell_api_token_v4 脚本错误次数已达到 $max_retries 次
